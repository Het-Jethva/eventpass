...
describeWithDatabase("Admission application service", () => {
  ...
  it("replays the stored online outcome when the same clientAttemptId is submitted twice", async () => {
    const rollback = Symbol("rollback online admission idempotency test");
    try {
      await database.transaction(async (transaction) => {
        const service = createAdmissionApplicationService({
          database: transaction as unknown as typeof database,
        ...
        const [staffUser] = await transaction
          .insert(user)
          .values({
            name: "Gate Volunteer Replay",
            email: `volunteer-replay-${randomUUID()}@example.com`,
            emailVerified: true,
          })
          .returning({ id: user.id });
        const [createdEvent] = await transaction
          .insert(event)
          .values({
            name: "Online admission idempotency test",
            description: "Exercises replay of a stored Scan Attempt.",
            slug: `admission-replay-${randomUUID()}`,
            status: "published",
            eventTimeZone: "UTC",
            startsAt: new Date("2030-01-02T12:00:00.000Z"),
            endsAt: new Date("2030-01-02T14:00:00.000Z"),
        ...
        const [createdRegistration] = await transaction
          ...
          .values({
            eventId: createdEvent!.id,
            attendeeName: "Ada Lovelace",
            email: "ada-replay@example.com",
            normalizedEmail: "ada-replay@example.com",
            status: "confirmed",
            capacityOutcome: "capacity_hold",
            verifiedAt: new Date("2030-01-01T12:00:00.000Z"),
          })
        ...
        const first = await service.admitOnline(attempt);
        const replay = await service.admitOnline(attempt);
        ...
        expect(
          await transaction
            .select()
            .from(scanAttempt)
            .where(eq(scanAttempt.id, clientAttemptId)),
        ).toHaveLength(1);
        throw rollback;
      });
    } catch (error) {
