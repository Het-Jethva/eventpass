...
describeWithDatabase("Ticket application service", () => {
  ...
  afterAll(async () => {
    if (eventIds.length > 0) {
      ...
      await database.delete(emailDelivery).where(inArray(emailDelivery.eventId, eventIds));
  ...
  it("stops honoring a management link once a resend has rotated it", async () => {
    const held = await createHeldRegistration();
    const verified = await service.verifyRegistration(held.event.slug, held.verificationToken);
    expect(verified.outcome).toBe("confirmed");
    if (verified.outcome !== "confirmed") throw new Error("Expected confirmation.");
    ...
    if (resent.outcome !== "sent") throw new Error("Expected the Ticket to be resent.");
    expect(resent.managementToken).not.toBe(verified.managementToken);
    ...
    expect(await service.getManagementView(resent.managementToken!)).not.toBeNull();
  });

  it("refuses a fourth Ticket email within the hour", async () => {
    const held = await createHeldRegistration();
    const verified = await service.verifyRegistration(held.event.slug, held.verificationToken);
    expect(verified.outcome).toBe("confirmed");
    if (verified.outcome !== "confirmed") throw new Error("Expected confirmation.");

    // The service counts Email Delivery rows, which the real sender records
    // and the test double does not, so stand in for three sends this hour.
